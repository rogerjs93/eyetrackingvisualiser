...
Converts CSV format to training-ready feature matrices

Supported CSV Formats:
1. Point of Regard Format (Kaggle Autism Dataset):
   - Point of Regard Right X [px], Point of Regard Right Y [px]
   - Individual files per participant (1.csv, 2.csv, etc.)
   
2. Pupil Size Format (Alternative):
...
def load_eyetracking_csv(csv_path):
    """
    Load CSV with Point of Regard or Pupil Size format (auto-detect)
    
    Supported formats:
    1. Point of Regard Right X/Y [px] (Kaggle dataset)
    2. Pupil Size Right X/Y [px] (Alternative format)
    """
    print(f"\n{'='*60}")
    ...
    # Auto-detect format
    if 'Point of Regard Right X [px]' in df.columns:
        x_col = 'Point of Regard Right X [px]'
        y_col = 'Point of Regard Right Y [px]'
        time_col = 'RecordingTime [ms]'
        print(f"📊 Detected: Point of Regard format")
    elif 'Pupil Size Right X [px]' in df.columns:
        x_col = 'Pupil Size Right X [px]'
        y_col = 'Pupil Size Right Y [px]'
        time_col = 'recordingtime [ms]'
        print(f"📊 Detected: Pupil Size format")
    else:
        x_col = 'Pupil Size Right X [px]'
        y_col = 'Pupil Size Right Y [px]'
        time_col = 'recordingtime [ms]'
...
def load_participant_directory(directory_path):
    """
    Load all participant CSV files from a directory
    (For Kaggle dataset with 1.csv, 2.csv, etc.)
    """
    ...
    for csv_file in csv_files:
        participant_id = csv_file.stem  # e.g., '1', '2', '3'
        df = pd.read_csv(csv_file)
        df['participant_id'] = participant_id
        all_data.append(df)
    
    combined_df = pd.concat(all_data, ignore_index=True)
    print(f"✅ Combined {len(combined_df)} total rows from {len(csv_files)} participants")
...
def split_by_participant_and_trial(df):
    ...
    Each participant = 1 training sample (aggregates all their trials)
    """
    print(f"\n{'='*60}")
    print("Splitting by participant")
    print(f"{'='*60}")
    
    # Try different column names for participant
    participant_col = None
    for col_name in ['participant_id', 'Participant', 'participant']:
        if col_name in df.columns:
            participant_col = col_name
            break
    ...
    if participant_col:
        ...
        print("⚠️ No participant column found. Treating entire dataset as 1 sample.")
        grouped = [('sample', df)]
    
    samples = []
    for name, group in grouped:
        if len(group) < 100:  # Skip very short sequences (need minimum data)
            print(f"⚠️ Skipping {name}: only {len(group)} points (need ≥100)")
            continue
...
def main():
    ...
    # Step 1: Load CSV (auto-detect single file or directory)
    csv_path_obj = Path(csv_path)
    if csv_path_obj.is_dir():
        # Directory of participant files
        df, csv_files = load_participant_directory(csv_path)
        # Auto-detect column names from first file
        if 'Point of Regard Right X [px]' in df.columns:
            x_col = 'Point of Regard Right X [px]'
            y_col = 'Point of Regard Right Y [px]'
            time_col = 'RecordingTime [ms]'
        ...
        # Single CSV file
        df, x_col, y_col, time_col = load_eyetracking_csv(csv_path)
